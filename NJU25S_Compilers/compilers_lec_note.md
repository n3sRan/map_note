...
*介绍了 RISC-V 汇编语言的语法, 有 ICS 和 PA 基础可放心跳过😇.*
...
- LR(k) 的优点: 看到某个产生式的整个右部对应的词法单元之后再决定
...
- L: **从左向右** (Left-to-right) 扫描输入
- R: 构建**反向** (Reverse) **最右** (Rightmost) 推导
- 状态
  - 在任意时刻, 语法分析树的**上边缘** (红框) 与**剩余的输入** (蓝框) 构成当前句型
  - LR 语法分析器使用**栈**存储语法分析树的上边缘
    - 栈操作: **移入输入符**号 与 **按产生式归约**
    - 直到栈中仅剩开始符号 E, 且输入已结束, 则成功停止
  - ![](../0_Attachment/Pasted%20image%2020250425150115.png)
...
LR 分析表: 在当前**状态** (编号)下, 面对**当前文法符号**时, 该采取什么动作

- ![](../0_Attachment/Pasted%20image%2020250425153706.png)
- 状态是语法分析树的上边缘, 存储在**栈**中 (栈中存储语法分析器的状态 (编号), "编码"了语法分析树的上边缘)
- 规约的必要条件: 当前状态中, 已观察到某个产生式的**完整右部**

句柄 (Handle)

- 在输入串的 (唯一) 反向最右推导中, 如果下一步是逆用产生式 $A → \alpha$, 将 $\alpha$ 归约为 $A$, 则称 $\alpha$ 是当前句型的句柄.
- LR 语法分析器的关键就是高效寻找每个归约步骤所使用的**句柄**

LR(0) 句柄识别有穷状态自动机 (Handle-Finding Automaton)

- ![](../0_Attachment/Pasted%20image%2020250425154535.png)
- 其状态集可以表示为一个项集族
- 定义见PPT: LR(0) 项, 项集, 项集族, 增广文法

通过 LR(0) 句柄识别自动机构建 LR(0) 分析表

- 规则: ![](../0_Attachment/Pasted%20image%2020250425154717.png)
- 分析表 (非 LR(0), 因为有冲突): ![](../0_Attachment/Pasted%20image%2020250425154841.png)
- LR(0) 分析表每一行 (状态) 所选用的归约产生式是相同的, 0的含义: **归约**时不需要向前看

LR(0) 自动机与栈之间的互动关系: **自动机才是本质, 栈是实现方式**
...
问题: 如何消除上一节中非 LR(0) 文法的 "移入-规约" 冲突

解决: SLR(1) 分析表 (S 指 Simple)

- 并非无脑选择规约, 而是观察下一个非终结符再做决定
- 规约: ![](../0_Attachment/Pasted%20image%2020250505112615.png)

SLR(1) 文法

- 如果文法 G 的SLR(1) 分析表是无冲突的, 则 G 是 SLR(1) 文法
- 无冲突: ACTION 表中每个单元格最多只有一种动作
- 两类可能的冲突: 移入-归约 冲突, 归约-归约 冲突

仍然有非 SLR(1) 文法

- 这说明仅仅判断 FOLLOW, 是一种"粗糙"的静态估计
- 我们希望LR 语法分析器的每个状态能尽可能精确地指明哪些输入符号可以跟在句柄 $A → α$ 的后面
- 在 LR(0) 自动机中, 某个项集 $I_j$ 中包含 $[A → \alpha ·]$, 则在之前的某个项集 $I_i$ 中包含 $[B → \beta · A\gamma]$ 与 $[A → · \alpha]$, 这表明 只有 $a \in FIRST(\gamma)$ 时, 才可以进行 $A → α$ 规约

### LR(1)

改造: LR(1) 项

- 定义: ![](../0_Attachment/Pasted%20image%2020250505113527.png)
- $[A → \gamma ·, a]$: 只有下一个输入符号为 a 时, 才可以按照 $A → \gamma$ 进行归约
- 对 $I$ 求闭包: ![](../0_Attachment/Pasted%20image%2020250505114124.png)
- 状态迁移: ![](../0_Attachment/Pasted%20image%2020250505114144.png)
- 初始状态: $CLOSURE ([S' → ·S,\$])$

LR(1) 分析表

- 构建规则: ![](../0_Attachment/Pasted%20image%2020250505114411.png)
- LR(1) 文法: 如果文法 G 的LR(1) 分析表是无冲突的, 则 G 是 LR(1) 文法

*示例见 PPT*

总结: LR(0), SLR(1), LR(1) 的归约条件: ![](../0_Attachment/Pasted%20image%2020250505114546.png)

### LALR(1)

LR(1) 缺点: 虽然性能强大, 但生成的 LR(1) 分析表可能过大, 状态过多, 比较吃性能

解决: **LALR(1)**

- 合并具有相同核心 LR(0)项的状态 (忽略不同的向前看符号)
- LALR(1) 分析表不会引入移入/归约冲突 (反证法证明)
- LALR(1) 分析表可能会引入归约/归约冲突

优点

- 状态数量与 SLR(1) 语法分析器的状态数量**相同**
- 不会产生移入/归约冲突

### 二义性文法

好消息: 善用 LR 语法分析器, 可以处理二义性文法
...
- 核心任务: 将 LLVM 中间表示转换为特定目标架构的机器指令
- 这是一个**组合优化**问题 (也是 NP 问题)
- LLVM 采用基于 SelectionDAG 的选择器

目标代码生成流程

- ![](../0_Attachment/Pasted%20image%2020250506151602.png)
- ![](../0_Attachment/Pasted%20image%2020250506151058.png)
- Passes: 优化
- **Instruction Selection**
  1. LLVM IR → SelectionDAG
  2. SelectionDAG → MachineInstr, 通过合法化 DAG (Legalization), 以及指令选择 (Pattern Matching) 实现
  3. ![](../0_Attachment/Pasted%20image%2020250506151749.png)
  4. ![](../0_Attachment/Pasted%20image%2020250506151753.png)
- **Prologue/Epilogue Insertion** (函数序/尾): 发生在 Register allocation 之后
...
- 最佳覆盖方案: 不存在可以合并成更小代价树型的相邻树型
- 最优覆盖方案: 给指令选择加上权重, 使用动态规划思想计算权重最小的方案
  - ![](../0_Attachment/Pasted%20image%2020250506152349.png)

## 17 目标代码生成-寄存器分配

> 基于 Hunyuan T1 整理

将使用无限虚拟寄存器的程序 $P_v$ 映射到具有有限物理寄存器的程序 $P_p$

- 若物理寄存器不足, 部分虚拟寄存器需映射到内存, 称为溢出虚拟寄存器 Spilled Virtuals
- 组合优化问题
- 关键: 在同一个程序点上活跃的变量是有冲突的, 不能分配到同一个寄存器
...
- **活跃**: 对于给定的变量 x, 考虑从其一个定义点 p 到使用点 q 的路径 l. 对于该路径 l 上的任意点 r, 如果 r 和 q 之间没有对变量 x 的其它定义, 则称 x 在程序点 r 上是活跃的
- **活跃分析**: 分析变量的活跃点的程序分析被称为活跃分析
- LiveIn(s) : s 执行前的活跃变量集合, LiveOut(s) : s 执行后的活跃变量集合
- 数据流方程组 (通过不动点求解): ![](../0_Attachment/Pasted%20image%2020250506155204.png)
...
- 操作
  ...
  - 溢出: 无可用寄存器时存储到内存
- 改进
  - 预留物理寄存器: 预留给溢出变量临时使用, 如RISC-V典型程序预留2个
  - 内存-寄存器交换: 通过临时变量减少访存次数
